...
//import ThankYouPage from "../landingPages/Thankyou";
...
export default function Feedback() {
    ...
    const submitData = (e) => {
        ...
        // fetch("http://localhost:8081/feedback?service_id="+1+"&user_id="+1, reqOption)
        
        fetch("http://localhost:8081/feedback?service_id="+service_id+"&user_id="+user_id, reqOption)
            .then(resp => resp.text())
            .then(str => navigate('/custWelcome'))
    ...
    return (
        <div className="container mt-4">
            ...
            <form name="feedback_form" id="feedback_form" onSubmit={submitData}>
                ...
                <div className="mb-3 d-flex flex-row py-1 " id="rating">
                    {[1, 2, 3, 4, 5].map((value) => (
                        <div key={value} className="form-check mx-3">
                            <input
                                className="form-check-input bg-outline-info"
                                type="radio"
                                name="rating"
                                id={`rating_${value}`}
                ...
                <button type="submit" className="btn btn-outline-info rounded-pill px-3" onClick={(e)=>{submitData(e)}}>Post</button>
                <button className="btn btn-outline-info rounded-pill px-3 mx-3" type="button" onClick={() => navigate('/custWelcome')}>Go Back</button> {/* Replaced anchor tag with a button and added onClick event */}
            </form>
        </div>
    );
}